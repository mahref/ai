...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        allPossible = util.Counter()
        for p in self.legalPositions:
            pDist = self.getPositionDistribution(self.setGhostPosition(gameState, p))
            for position, probability in pDist.items():
                allPossible[position] += self.beliefs[p] * probability
        allPossible.normalize()
        self.beliefs = allPossible
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        self.particles = []
        share = int(self.numParticles / len(self.legalPositions))
    ...
    def elapseTime(self, gameState):
        ...
        a belief distribution.
        """
        allDist = [self.getPositionDistribution(self.setGhostPosition(gameState, p)) for p in self.particles]
        self.particles = [util.sample(dist) for dist in allDist]
